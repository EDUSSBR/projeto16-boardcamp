...
export async function getRentalsController(req, res) {
    try {
        ...
        if (queryGameID || queryCustomerID || offset || limit || orderedColumnExists || !thereIsNoStatus || desc==="DESC") {
            ...
            `
            if (queryCustomerID || queryGameID || status){
                query+=`WHERE (`
            }
            if (queryCustomerID && queryGameID){
                query+=`(c.id=$1 AND g.id=$2)`
            ...
            if (status==='open'){
                ...
                query+=`(r."returnDate" IS NULL))` 
            }  else if (status==='closed'){
                query+=`(r."returnDate" IS NOT NULL))` 
            } else {
                query+=")"
            }
            ...
            SELECT r.id, r."customerId", r."gameId", r."rentDate", r."daysRented", r."returnDate", r."originalPrice", r."delayFee", c.name AS "customerName", g.name AS "gameName" 
            FROM rentals r  
            JOIN customers c ON r."customerId"=c.id 
            JOIN games g ON g.id=r."gameId";
            `)
